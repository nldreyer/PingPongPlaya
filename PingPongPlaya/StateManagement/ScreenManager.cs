...
namespace PingPongPlaya.StateManagement
{
    /// <summary>
    ...
    public class ScreenManager : DrawableGameComponent
    {
        ...
        /// <summary>
        /// Removes a screen from the ScreenManager, adds a new one, and activates the new one.
        /// </summary>
        /// <param name="remove">Screen to remove</param>
        /// <param name="add">Screen to add</param>
        public void RemoveAddScreen(GameScreen remove, GameScreen add, PlayerIndex? controllingPlayer)
        {
            add.ControllingPlayer = controllingPlayer;
            add.ScreenManager = this;
            add.IsExiting = false;

            // If we have a graphics device, tell the screen to unload its content 
            if (_isInitialized) remove.Unload();

            _screens.Remove(remove);
            _tmpScreensList.Remove(remove);

            // If we have a graphics device, tell the screen to load content
            if (_isInitialized) add.Activate();
